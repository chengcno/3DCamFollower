...
void Cam::CreatPartMesh()
{
    ...
    //cout<<"rows = "<<_groove->MinusMesh->verM.rows()<<endl;
    camMesh = meshBoolean.MeshMinus(camMesh, _groove->MinusMesh);
    //camMesh->saveOBJ("0405Test1.obj");
...
/*void Cam::AddLhandle()
{
    ...
    /// handle = cy1 + cy2
    Mesh* cyMesh = meshCreator.CreateCylinder( Vector3d(0,0,zmin_-0.45),
                                               Vector3d(0.5,0,zmin_-0.45), 0.05, _CSG_MESH_NUM_N);

    Mesh* tmp_mesh = meshBoolean.MeshUnion(mesh,cyMesh);
    delete mesh; delete cyMesh;

    cyMesh = meshCreator.CreateCylinder( Vector3d(0.5,0,zmin_-0.45 - 0.75),
                                         Vector3d(0.5,0,zmin_-0.45 + 0.05), 0.05, _CSG_MESH_NUM_N);

    Mesh* Tmp_mesh = meshBoolean.MeshUnion(tmp_mesh,cyMesh);
    delete tmp_mesh; delete cyMesh;


    /// marker
    cyMesh = meshCreator.CreateCylinder( Vector3d(0,0,zmin_-0.5),
                                         Vector3d(0,_CAM_AXIS_RADIUS,zmin_-0.5), 0.01,6);
    tmp_mesh = meshBoolean.MeshMinus(Tmp_mesh,cyMesh);
    delete Tmp_mesh; delete cyMesh;


    /// resist ring
    cyMesh = meshCreator.CreateCylinder( Vector3d(0,0,zmax_ + 0.25 + _GAP_TOR_LARGE),
                                         Vector3d(0,0,zmax_ + 0.30 + _GAP_TOR_LARGE), _CAM_AXIS_RADIUS + 0.03,_CSG_MESH_NUM_N);

    Tmp_mesh = meshBoolean.MeshUnion(tmp_mesh,cyMesh);
    delete tmp_mesh; delete cyMesh;

    cyMesh = meshCreator.CreateCylinder( Vector3d(0,0,zmin_ - 0.25 - _GAP_TOR_LARGE),
                                         Vector3d(0,0,zmin_ - 0.30 - _GAP_TOR_LARGE), _CAM_AXIS_RADIUS + 0.03,_CSG_MESH_NUM_N);

    tmp_mesh = meshBoolean.MeshUnion(Tmp_mesh,cyMesh);
    delete Tmp_mesh; delete cyMesh;

    tmp_mesh->GetMeshTriangles();
    mesh = tmp_mesh->DeepCopy();
    delete tmp_mesh;
}
*/
...
void Cam::ComputeCrossAngle() {
    ...
    for (int i = 0; i < Lines; i++) {
        ...
        for (int j = 0; j < Size; j++) {
            ...
            int Lnum = 13;
/*
            for (int k = 0; k < 2 * Lnum; k++) {
                Vector3d LineP;
                double cita;
                if (k < Lnum) cita = -1.1 + k * 2.2 / (Lnum - 1);
                else cita = -1.1 + (k - Lnum) * 2.2 / (Lnum - 1) + M_PI;
                //cita = k*M_PI/(Lnum);
                /// rFol = 2.25
                LineP = 2.25 * (cos(cita) * LineN1 + sin(cita) * LineN2) + LinePO;
                double t1, t2;
                if (LineToCylinder(LineP, LineVec, CyP, CyT, rCFball, t1, t2)) {
                    if (t2 > 0.85 * rCFball) {
                        Vector3d LcrossCy = LineP + LineVec * t2;
                        double paraCy = (LcrossCy - CyP).dot(CyT.normalized());
                        //todo: adjust 1.2 scale
                        if (paraCy <= CyT.norm() * 1.0 && paraCy >= -CyT.norm() * 0.00001) {
                            //if (!tag) InsectPoly.push_back(LcrossCy);
                            Vector3d ProCross = (LcrossCy - CyP) - paraCy * (CyT.normalized());
                            //cout<<"r= "<<ProCross.norm()<<" |=" <<ProCross.dot(CyT)<<" TN="<<CyT.dot(CyN)<<endl;
                            ProCross.normalize();
                            CyT.normalize();
                            double deg = getDegree(ProCross.dot(CyN), ProCross.dot(CyN.cross(CyT)));
                            //CrAngles[j].push_back(deg);
                            alphas[j] = _MAX(alphas[j], deg);
                            betas[j] = _MIN(betas[j], deg);
                            tag = true;
    ...
    for (int i = 0; i < Size; i++) {
        ...
        if (i < Size - 2) {
            ...
            if (abs(alphas[i] - 2 * alphas[i + 1] + alphas[i + 2]) > 2.5 * angleTor) {
                ...
                //CorssAngleUp[i+1] = _MAX(CorssAngleUp[i], CorssAngleUp[i+1]) + _Groove_Open_Angle_Tor;
            }
            if (abs(betas[i] - 2 * betas[i + 1] + betas[i + 2]) >
                ...
                if (betas[i] - betas[i + 1] < -1.5 * angleTor) {
                    ...
                    //CorssAngleLow[i+1] = CorssAngleLow[i];
                }
                ...
                if ((Pnow - Pnew).norm() >= 2 * (CyP - CyP2).norm()) {
                    ...
                    ProCross.normalize();
                    double deg = getDegree(ProCross.dot(CyN2), ProCross.dot(CyN2.cross(CyT2)));
                    //CorssAngleLow[i+1] = deg - 0.1*_Groove_Open_Angle_Tor;
                    cout << "Low angle i " << i << " or " << i + 1 << " may fail\n";

                }
            }
        }
        //if(SomeWraning) printf("Cut Angle Differs, Waring!\n");
    }
    //cout<<"up340="<<alphas[340]<<" up341="<<alphas[341]<<" up342 = "<<alphas[342]<<" up343="<<alphas[343]<<endl;
    //todo: NOT ENOUGH
