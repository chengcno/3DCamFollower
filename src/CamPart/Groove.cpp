...
int Groove::CreateverList(double alpha, double beta)
{
    ...
    for(int itr=0;itr<NSize;itr++)
    {
        ...
        //Eigen::Vector3d EdgeStart = radius_*cos(alpha)*N_1 + radius_*sin(alpha)*N_2 + pitchCurve[itr];
        //Eigen::Vector3d EdgeStart1 = EdgeStart + N_1*_THICKNESS_of_Groove_edge;
        ...
        //Eigen::Vector3d EdgeEnd = radius_*cos(beta)*N_1 + radius_*sin(beta)*N_2 + pitchCurve[itr];
        //Eigen::Vector3d EdgeEnd1 = EdgeEnd + N_1*_THICKNESS_of_Groove_edge;
        Eigen::Vector3d EdgeEnd2 = EdgeEnd1 - vecOrient*radius_;

        G_verList.push_back(EdgeEnd1);
        MG_verList.push_back(EdgeEndAno);

        //GrooveMesh_->verList.push_back(EdgeEnd2);
        if(itr == 0) additionalSize += 1; //do once
    }
