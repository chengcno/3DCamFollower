...
void IOMechs::ReadModels()
{
    ...
    for(int i=0;i<sizeMesh;i++)
    {
        ...
        if(i == 0)
        {
            double L = (cM.colwise().maxCoeff() - cM.colwise().minCoeff()).norm();
            cout<< "L = " << L <<endl;
            scaleL = 200.0/L;
        }
        partMesh->TransformMesh(GetScaleMatrix(Vector3d(scaleL, scaleL, scaleL)));
        meshList.push_back(partMesh);
        cout<< "read " << i << "st model, with V = " << partMesh->verM.rows() << ", F = " << partMesh->triM.rows() <<endl;
    }
}
