...
void ReadCurve::readFile(string fname)
{
    ...
    fileC >> SurType;
    if(SurType == 0)
        surface_type = SurfaceType::R;
    if(SurType == 1)
        surface_type = SurfaceType::T;
    if(SurType == 2)
        surface_type = SurfaceType::RR;
    if(SurType == 3)
    ...
    if(surface_type == SurfaceType::RR)
    {
        for(int i=0;i<pNum;i++)
        {
            double x = posM(i, 0);
            double y = posM(i, 1);
            posM(i, 0) = -sqrt(radiusR*radiusR - y*y - x*x);
            posM(i, 2) = x;

            folBetas.push_back(-asin(posM(i, 1)/radiusR));
            folAlphas.push_back(-atan(posM(i,2)/posM(i,0)));
        }
    }
    else if(surface_type == SurfaceType::RT)
    {
        for(int i=0;i<pNum;i++)
        {
            double x = posM(i, 0);
            double y = posM(i, 1);
            posM(i, 0) = -sqrt(radiusR* radiusR - y*y);
            posM(i, 2) = x;

            folAlphas.push_back(posM(i, 2));
            folBetas.push_back(atan(posM(i,1)/posM(i,0)));
        }
/*
        _bsF = new BSplineFit();
        for(int i=0;i<pNum;i++)
        ...
        camRT->folCen = folCen;
        camRT->folAxis << 0, 0, 1;
        camRT->pJoint_0<< -40, 0, 0;
        camRT->pAnkle_0<< -60, 0, 0;
        camRT->folTheta = folBetas;
        camRT->folTrans = folAlphas;
        camRT->CreateCamMesh();
        camRT->CreateFolMesh();
        */
    }
...
void ReadCurve::CreateCamFollower()
{
    ...
    if(surface_type == SurfaceType::RR)
    {
        ...
        auto folJoint = new Joint('u');
        folJoint->CreateSimpleModel();
        folJoint->InitPose(0,1, GetTranslateMatrix(folCen)* GetRotationMatrix(Vector3d(0, M_PI_2, 0)));

        cam2R->folMesh = meshBoolean.MeshConnect(cam2R->folMesh, folJoint->partA);
        Mesh* endE = meshCreator.CreateCylinder(folCen, folCen-Vector3d(radiusR, 0, 0), 1.8, CSG_NUM_M);
        cam2R->folMesh = meshBoolean.MeshConnect(cam2R->folMesh, endE);

        Mesh* camSup = meshCreator.CreateCuboid(Vector3d(20, 20, 8));
        Mesh* camAxis = meshCreator.CreateCylinder(Vector3d(0,0,-40), Vector3d(0, 0, 40), 6, CSG_NUM_M);
        cam2R->camMesh = meshBoolean.MeshConnect(cam2R->camMesh, camAxis);
        supMesh = meshCreator.CreateCuboid(Vector3d(20, 20, 8));
        supMesh->TransformMesh(GetTranslateMatrix(Vector3d(0,0,30)));
        camSup->TransformMesh(GetTranslateMatrix(Vector3d(0,0,-30)));
        supMesh = meshBoolean.MeshConnect(supMesh, camSup);
        supMesh = meshBoolean.MeshConnect(supMesh, folJoint->partB);
    }
...
void ReadCurve::AppendReadMechs(iglViewer &viewer)
{
    ...
    partMech.set_colors(RowVector3d (0.75, 0.45, 0.89));
...
void ReadCurve::UpdateMotion(iglViewer &viewer, int frame)
{
    int N = frame % pNum;
    MatrixXd verM;
    if(surface_type == SurfaceType::RR)
        cam2R->camMesh->TransformMesh(GetRotationMatrix(N*2.0*M_PI/pNum, Vector3d(0, 0, 1)), verM);
    else if(surface_type == SurfaceType::RT)
        camRT->camMesh->TransformMesh(GetRotationMatrix(N*2.0*M_PI/pNum, Vector3d(0, 0, 1)), verM);
    int Si = viewer.data_list.size();
    viewer.data_list[Si - 2].set_vertices(verM);
    viewer.data_list[Si - 2].compute_normals();

    MatrixXd verM2;
    if(surface_type == SurfaceType::RR)
        cam2R->folMesh->TransformMesh(GetRotationMatrix(folAlphas[N], cam2R->folAxisAlpha, cam2R->folCen)
        * GetRotationMatrix(folBetas[N], cam2R->folAxisBeta, cam2R->folCen), verM2);
    else if(surface_type == SurfaceType::RT)
        camRT->folMesh->TransformMesh(GetRotationMatrix(folBetas[N], camRT->folAxis, camRT->folCen)
        * GetTranslateMatrix(camRT->folAxis*folAlphas[N]), verM2);
    viewer.data_list[Si - 1].set_vertices(verM2);
    viewer.data_list[Si - 1].compute_normals();

}

void ReadCurve::AppendOptMechs(iglViewer &viewer)
{
    iglViewerData partMech;
    partMech.clear();
    partMech.set_mesh(supMesh->verM, supMesh->triM);
    partMech.set_colors(RowVector3d(0.85, 0.85, 0.85));
    partMech.face_based = true;
    partMech.show_lines = unsigned (0);
    partMech.compute_normals();
    viewer.data_list.push_back(partMech);
    ...
    partMech.clear();
    if(surface_type == SurfaceType::RR)
        partMech.set_mesh(cam2R->camMesh->verM, cam2R->camMesh->triM);
    else if(surface_type == SurfaceType::RT)
        partMech.set_mesh(camRT->camMesh->verM, camRT->camMesh->triM);
    partMech.set_colors(RowVector3d (0.75, 0.9, 0.9));
    partMech.face_based = true;
    partMech.show_lines = unsigned (0);
    partMech.compute_normals();
    viewer.data_list.push_back(partMech);
    ...
    partMech.clear();
    if(surface_type == SurfaceType::RR)
        partMech.set_mesh(cam2R->folMesh->verM, cam2R->folMesh->triM);
    else if(surface_type == SurfaceType::RT)
        partMech.set_mesh(camRT->folMesh->verM, camRT->folMesh->triM);
    partMech.set_colors(RowVector3d (0.93, 0.77, 0.65));
    partMech.face_based = true;
    partMech.show_lines = unsigned (0);
    partMech.compute_normals();
    viewer.data_list.push_back(partMech);
